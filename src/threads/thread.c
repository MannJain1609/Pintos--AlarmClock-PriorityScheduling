...
/* List of sleeping threads. We will add a thread to this list when it calls timer_sleep()*/
...
thread_init (void) 
{
  ...
  list_init (&all_list);
  list_init (&sleep_list);/*initiated the new list*/
...
thread_update_priority(struct thread *t)
{
  ...
  {
    int lock_priority = list_entry(list_max(&t->locks, compare_locks_by_priority,NULL),
                        struct lock, elem)->max_priority;
    /*If the waiting threads in lock has a higher priority than the current thread, 
    there will be priority donation*/
